/*********************************************************************
 *
 * $Id: yocto_serialport.js 75578 2026-08-17 17:01:23Z seb $
 *
...
async function yInternalEventCallback(YSerialPort_obj, str_value)
{
...
class YSerialPort extends YFunction
{
    ...
    async _xymodemProcess(timeoutSec)
    {
        ...
        sentBytes = this._yapi.imm_atoi(this.imm_json_get_key(json, 'sent'));
        ...
        return parseInt((100 * sentBytes) / this._xytotal, 10);
    }
     ...
     *
     * @param buff {Uint8Array} : the binary buffer to send
     * @param timeoutSec {number} : the timeout before aborting send (e.g. 60 sec)
     *
     ...
     *
     * @param filename {string} : the filename associated with the data in the buffer
     * @param buff {Uint8Array} : the binary buffer to send
     * @param timeoutSec {number} : the timeout before aborting send (e.g. 60 sec)
     *
     * @return {Promise<number>} YAPI.SUCCESS if the call succeeds.
...
class YSerialPortProxy extends YFunctionProxy
{
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    xmodemUpload(buff,timeoutSec)
    {
        this.liveFunc.xmodemUpload(buff, timeoutSec);
        return YAPI_SUCCESS;
    }
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    xmodem1kUpload(buff,timeoutSec)
    {
        this.liveFunc.xmodem1kUpload(buff, timeoutSec);
        return YAPI_SUCCESS;
    }
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    ymodemUpload(filename,buff,timeoutSec)
    {
        this.liveFunc.ymodemUpload(filename, buff, timeoutSec);
        return YAPI_SUCCESS;
    }
