...
class YRfidReader extends YFunction
{
    ...
    async get_tagIdList()
    {
        ...
        if ((json).length > 3) {
            ...
            for (let ii_0 of jsonList) {
                taglist.push(this.imm_json_get_string(this._yapi.imm_str2bin(ii_0)));
            }
        }
        return taglist;
    }
     ...
     *
     * @param tagId {string} : identifier of the tag to use
     * @param ndefType {string} : the content type, either "U" for a URL, or a
     *         generic MIME type like "text/vcard"
     * @param payload {Uint8Array} : the payload of the NDEF record
     * @param options {YRfidOptions} : an YRfidOptions object with the optional
     *         command execution parameters, such as security key
     *         if required
     * @param status {YRfidStatus} : an RfidStatus object that will contain
    ...
    async tagWriteBinNFC(tagId,ndefType,payload,options,status)
    {
        ...
        nfcType = await tagInfo.get_tagNFCtype();
        if (!(nfcType == 5)) {
            return this._throw(this._yapi.INVALID_ARGUMENT,'no NFC support for this tag',this._yapi.INVALID_ARGUMENT);
        }
        usableSize = await tagInfo.get_tagUsableSize();
        ccLen = 4;
        nfcBlocks = parseInt((usableSize - ccLen) / 8, 10);
        if (nfcBlocks > 255) {
            if (nfcType == 5) {
                ccLen = 8;
                nfcBlocks = parseInt((usableSize - ccLen) / 8, 10);
        ...
        if (ccLen == 4) {
            ...
            buff.set([0x00], 4);
            buff.set([0x00], 5);
            buff.set([parseInt(nfcBlocks / 256, 10)], 6);
        ...
        if (tlvHdrLen == 2) {
            ...
            buff.set([parseInt(ndefLen / 256, 10)], ccLen + 2);
        ...
        if (srBit > 0) {
            ...
            buff.set([0], absPos + 2);
            buff.set([0], absPos + 3);
            buff.set([parseInt(payloadLen / 256, 10)], absPos + 4);
        ...
        idx = await tagInfo.get_tagFirstBlock();
     ...
     *
     * @param tagId {string} : identifier of the tag to use
     * @param url {string} : the URL to write on the tag
     * @param options {YRfidOptions} : an YRfidOptions object with the optional
     *         command execution parameters, such as security key
     *         if required
     * @param status {YRfidStatus} : an RfidStatus object that will contain
     ...
     *
     * @param tagId {string} : identifier of the tag to use
     * @param ssid {string} : the SSID of the WiFi network to connect to
     * @param auth {string} : the network authentication type (currently always "WPA2")
     * @param secret {string} : the network password
     * @param options {YRfidOptions} : an YRfidOptions object with the optional
     *         command execution parameters, such as security key
     *         if required
     * @param status {YRfidStatus} : an RfidStatus object that will contain
     ...
     *         the detailled status of the operation
     *
     * @return {Promise<number>} the requested byte value (0...255)
     *
     * On failure, throws an exception or returns a negative error code. When it
     * happens, you can get more information from the status object.
     */
     ...
     * @param value {number} : the value to write (0...255)
     * @param options {YRfidOptions} : an YRfidOptions object with the optional
     *         command execution parameters, such as security key
     *         if required
     * @param status {YRfidStatus} : an RfidStatus object that will contain
     ...
     * @param password {string} : the password (16 characters hex string encoding 8 bytes)
     * @param options {YRfidOptions} : an YRfidOptions object with the optional
     *         command execution parameters, such as security key
     *         if required
     * @param status {YRfidStatus} : an RfidStatus object that will contain
...
class YRfidReaderProxy extends YFunctionProxy
{
     ...
     * happens, you can get more information from the status object.
     */
    tagWriteBinNFC(tagId,ndefType,payload,options,status)
    {
        this.liveFunc.tagWriteBinNFC(tagId, ndefType, payload, options, status);
        return YAPI_SUCCESS;
    }
     ...
     * happens, you can get more information from the status object.
     */
    tagWriteUrlNFC(tagId,url,options,status)
    {
        this.liveFunc.tagWriteUrlNFC(tagId, url, options, status);
        return YAPI_SUCCESS;
    }
     ...
     * happens, you can get more information from the status object.
     */
    tagWriteWifiConfigNFC(tagId,ssid,auth,secret,options,status)
    {
        this.liveFunc.tagWriteWifiConfigNFC(tagId, ssid, auth, secret, options, status);
        return YAPI_SUCCESS;
    }
     ...
     * happens, you can get more information from the status object.
     */
    tagGetConfigByte(tagId,addr,options,status)
    {
        this.liveFunc.tagGetConfigByte(tagId, addr, options, status);
        return YAPI_SUCCESS;
    }
     ...
     * happens, you can get more information from the status object.
     */
    tagSetConfigByte(tagId,addr,value,options,status)
    {
        this.liveFunc.tagSetConfigByte(tagId, addr, value, options, status);
        return YAPI_SUCCESS;
    }
     ...
     * happens, you can get more information from the status object.
     */
    tagSetPassword(tagId,passwordType,password,options,status)
    {
        this.liveFunc.tagSetPassword(tagId, passwordType, password, options, status);
        return YAPI_SUCCESS;
    }
