...
function makeIndex()
{
  ...
  let commonJS_index = 'require(\'./yocto_api.js\');\n';
  let ESmodule_index = 'export * from \'./yocto_api.js\';\n';
  fs.readdirSync(__dirname).forEach(function (mod) {
    if (mod.length > 3 && mod.slice(-3) == '.js' && mod != 'build.js' && mod != 'index.js' && mod != 'yocto_api.js') {
      commonJS_index += 'require(\'./' + mod.slice(0, -3) + '\');\n';
      ESmodule_index += 'export * from \'./' + mod.slice(0, -3) + '.mjs\';\n';
    }
  });
  fs.writeFileSync("index.js", commonJS_index, 'utf-8');
  fs.writeFileSync("../yoctolib_esm_nodejs/index.mjs", ESmodule_index, 'utf-8');
  fs.writeFileSync("../yoctolib_esm_html/index.mjs", ESmodule_index, 'utf-8');
...
function setVersion(str_newver)
{
  ...
  let esmjson = JSON.parse(fs.readFileSync('../yoctolib_esm_nodejs/package.json', 'utf8'));
  esmjson.version = newver;
  fs.writeFileSync('../yoctolib_esm_nodejs/package.json', JSON.stringify(esmjson, null, 2), 'utf-8');

  // update version number in yocto_api.js
  patchVersionInFile(newver, 'yocto_api.js');
  patchVersionInFile(newver, '../yoctolib_esm_html/yocto_api.mjs');
  patchVersionInFile(newver, '../yoctolib_esm_nodejs/yocto_api.mjs');
  ...
  [ 'example_html', 'example_nodejs', 'hidden'].forEach(function(dirname) {
    ...
    fs.readdirSync(lib).forEach(function (exname) {
      ...
      if (json && json.dependencies) {
        if(json.dependencies['yoctolib-es2017']) {
          json.dependencies['yoctolib-es2017'] = '^'+newver;
        }
        if(json.dependencies['yoctolib-esm']) {
          json.dependencies['yoctolib-esm'] = '^'+newver;
        }
