/*********************************************************************
 *
 * $Id: yocto_api.js 31697 2018-08-16 14:39:57Z seb $
 *
...
class YDevice
{
    ...
    {
        this._cache._expiration = this._yapi.GetTickCount() + this._yapi._cacheValidity;
    ...
    async requestAPI(int_msValidity)
    {
        ...
        if(!int_msValidity) {
            int_msValidity = this._yapi._cacheValidity;
        }
    ...
    {
        /** @type {YHTTPRequest} **/
        let yreq = await this.requestAPI(this._yapi._cacheValidity);
...
class YFirmwareUpdate
{
    ...
    {
        return await self.GetAllBootLoaders_internal();
    }
    ...
    {
        return await self.GetAllBootLoadersInContext_internal(yctx);
    }
    ...
    {
        return await self.CheckFirmware_internal(serial,path,minrelease);
    }
...
class YFunction
{
    ...
    async get_syncProxy()
    {
        if (this._cacheExpiration <= this._yapi.GetTickCount()) {
            try {
                await this.load(this._yapi._cacheValidity);
            } catch(e) {
    ...
    async get_logicalName()
    {
        ...
        let res;
        if (this._cacheExpiration <= this._yapi.GetTickCount()) {
            if (await this.load(this._yapi._cacheValidity) != YAPI_SUCCESS) {
    ...
    async _getAttr(str_attr)
    {
        if(this._cacheExpiration <= this._yapi.GetTickCount()) {
            ...
            if(await this.load(this._yapi._cacheValidity) != YAPI_SUCCESS) return null;
        }
    ...
    async isOnline()
    {
        ...
        /** @type {YFuncRequest} **/
        let yreq = await this._yapi.funcRequest(this._className, this._func, '', this._yapi._cacheValidity);
        if(yreq.errorType != YAPI_SUCCESS) {
        ...
        await this._parse(yreq, this._yapi._cacheValidity);
...
class YGenericHub
{
    ...
    async hubUpdateDeviceList()
    {
        ...
        try {
            ...
            /** @type {YHTTPRequest} **/
            let yreq = await hubDev.requestAPI(this._yapi._cacheValidity);
            if(yreq.errorType != YAPI_SUCCESS) {
                return yreq;
            }
...
//--- (generated code: YAPIContext constructor)
    ...
    async devRequest(str_device, str_request, obj_body, int_tcpchan)
    {
        ...
        if(str_device.substr(0,7) == 'http://' || str_device.substr(0,5) == 'ws://' || str_device.substr(0,6) == 'wss://') {
            ...
            if(lines[0] == 'GET /api.json') {
                return await lockdev.requestAPI(this._cacheValidity);
            }
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'1.10.31701';
    }
