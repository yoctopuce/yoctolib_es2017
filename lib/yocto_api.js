...
class YDataSet
{
    ...
    async processMore(progress,data)
    {
        ...
        tim = await stream.get_realStartTimeUTC();
        fitv = await stream.get_firstDataSamplesInterval();
    ...
    async get_measuresAt(measure)
    {
        ...
        for (let ii in this._streams) {
            if (await this._streams[ii].get_realStartTimeUTC() == startUtc) {
    ...
    async _parse(str_json)
    {
        ...
        for(var i = 0; i < loadval.streams.length; i++) {
            ...
            var streamStartTime = await stream.get_realStartTimeUTC();
            var streamEndTime = streamStartTime + await stream.get_realDuration();
            if(this._startTime > 0 && streamEndTime <= this._startTime) {
                ...
                if(stream.isClosed() && streamStartTime >= this._startTime &&
                    ...
                    summaryTotalAvg  += await stream.get_averageValue() * await stream.get_realDuration();
                    summaryTotalTime += await stream.get_realDuration();

                    var rec = new YMeasure(streamStartTime,
                        streamEndTime,
...
//--- (generated code: YAPIContext constructor)
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'1.10.33423';
    }
