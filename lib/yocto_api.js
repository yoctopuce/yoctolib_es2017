/*********************************************************************
 *
 * $Id: yocto_api.js 72340 2026-03-09 13:56:18Z seb $
 *
...
class YModule extends YFunction
{
    ...
    async get_allSettings()
    {
        ...
        if (await this.hasFunction('files')) {
            ...
            for (let ii_1 of filelist) {
                name = this.imm_json_get_key(this._yapi.imm_str2bin(ii_1), 'name');
    ...
    async set_extraSettings(jsonExtra)
    {
        ...
        for (let ii_0 of extras) {
            functionId = this.imm_get_json_path(ii_0, 'fid');
    ...
    async set_allSettingsAndFiles(settings)
    {
        ...
        if (await this.hasFunction('files')) {
            ...
            for (let ii_0 of files) {
                name = this.imm_get_json_path(ii_0, 'name');
                name = this.imm_decode_json_string(name);
                data = this.imm_get_json_path(ii_0, 'data');
    ...
    async set_allSettings(settings)
    {
        ...
        for (let ii_0 of old_dslist) {
            each_str = this.imm_json_get_string(this._yapi.imm_str2bin(ii_0));
        ...
        for (let ii_1 of new_dslist) {
            // remove quotes
            each_str = this.imm_json_get_string(this._yapi.imm_str2bin(ii_1));
            // split json path and attr
            leng = (each_str).length;
            eqpos = (each_str).indexOf('=');
            if ((eqpos < 0) || (leng == 0)) {
...
class YSensor extends YFunction
{
    ...
    async loadCalibrationPoints(rawValues,refValues)
    {
        ...
        rawValues.length = 0;
        refValues.length = 0;
        for (let ii_0 of this._calraw) {
            rawValues.push(ii_0);
        }
        for (let ii_1 of this._calref) {
            refValues.push(ii_1);
        }
        return YAPI_SUCCESS;
    }
...
//--- (generated code: YAPIContext constructor)
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'1.11.12413';
    }
