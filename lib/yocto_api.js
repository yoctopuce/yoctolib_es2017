/*********************************************************************
 *
 * $Id: yocto_api.js 33971 2019-01-11 14:10:11Z seb $
 *
    ...
    imm_triggerLogPull()
    {
        ...
        prom.then(async (yreq) => {
            ...
            try {
                ...
                console.error('Exception in device log callback:',e);
            }
            this._logIsPulling = false;
        });
    }
    ...
    async _invokeValueCallback(value)
    {
        if (this._valueCallbackFunction != null) {
            try {
                ...
                console.error('Exception in valueCallback:', e);
            }
        } else {
        }
        return 0;
    ...
    async _invokeConfigChangeCallback()
    {
        if (this._confChangeCallback != null) {
            try {
                ...
                console.error('Exception in configChangeCallback:', e);
            }
        }
        return 0;
    }
    ...
    async _invokeBeaconCallback(beaconState)
    {
        if (this._beaconCallback != null) {
            try {
                ...
                console.error('Exception in beaconCallback:', e);
            }
        }
        return 0;
    }
    ...
    async _invokeTimedReportCallback(value)
    {
        if (this._timedReportCallbackSensor != null) {
            try {
                ...
                console.error('Exception in timedReportCallback:', e);
            }
        } else {
        }
        return 0;
...
class YSSDPManager
{
    ...
    async _invokeCallback(str_serial, str_addUrl, str_removeUrl)
    {
        if(this._callback) {
            try {
                ...
                console.error('Exception in hub discovery callback:', e);
            }
        }
    }
    ...
    async ySSDPParseMessage(str_msg)
    {
        ...
        let values = {};
        if(lines[0] === SSDP_HTTP || lines[0] === SSDP_HTTP) {
    ...
    async ySSDPStart(func_callback)
    {
        ...
        for(let key in networkInterfaces) {
            ...
            for(let idx = 0; idx < iface.length; idx++) {
                ...
                request_sock.bind(0, ipaddr, () => {
                    // send initial discovery when interface is bound
                    request_sock.send(this.YSSDP_DISCOVERY_MSG, this.YSSDP_PORT, this.YSSDP_MCAST_ADDR_STR);
                });
                let notify_sock = this._dgram.createSocket({ type: 'udp4', reuseAddr: true });
                notify_sock.on('message', (msg, info) => {
                    this.ySSDPParseMessage(msg.toString());
                });
                notify_sock.bind(this.YSSDP_PORT, this.YSSDP_MCAST_ADDR_STR, () => {
...
//--- (generated code: YAPIContext constructor)
                            ...
                            case '+':
                                if (this._arrivalCallback != undefined) {
                                    try {
                                        ...
                                        console.error('Exception in device arrival callback:',e);
                                    }
                                }
                                break;
                            case '/':
                                if (this._namechgCallback != undefined) {
                                    try {
                                        ...
                                        console.error('Exception in device change callback:',e);
                                    }
                                }
                                break;
                            case '-':
                                if (this._removalCallback != undefined) {
                                    try {
                                        ...
                                        console.error('Exception in device removal callback:',e);
                                    }
                                }
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'1.10.33982';
    }
    ...
    async PreregisterHub(url, errmsg)
    {
        ...
        newhub.testHub(0, errmsg).then(() => { this._addHub(newhub); });
