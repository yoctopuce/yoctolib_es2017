/*********************************************************************
 *
 * $Id: yocto_api.js 75634 2026-08-20 08:59:13Z seb $
 *
...
class YDataStream
{
     ...
     *
     * @return {Promise<number>} an unsigned number corresponding to the run index.
     */
    async get_runIndex()
    {
        return this._runNo;
     ...
     *
     * @return {Promise<number>} an unsigned number corresponding to the number of seconds
     ...
     * of this data stream, this method returns 0.
     *
     * @return {Promise<number>} a floating-point number  corresponding to the number of seconds
     *         between the Jan 1, 1970 and the beginning of this data
     *         stream (i.e. Unix time representation of the absolute time).
     */
     ...
     *
     * @return {Promise<number>} an unsigned number corresponding to a number of milliseconds.
     */
     ...
     *
     * @return {Promise<number>} an unsigned number corresponding to the number of rows.
     *
     ...
     * if not yet done, which can cause a little delay.
     *
     * @return {Promise<number>} an unsigned number corresponding to the number of columns.
     *
     * On failure, throws an exception or returns zero.
     */
     ...
     * if not yet done, which can cause a little delay.
     *
     * @return {Promise<string[]} a list containing as many strings as there are columns in the
     *         data stream.
     *
     ...
     *
     * @return {Promise<number>} a floating-point number corresponding to the smallest value,
     ...
     * this method will always return YDataStream.DATA_INVALID.
     *
     * @return {Promise<number>} a floating-point number corresponding to the average value,
     *         or YDataStream.DATA_INVALID if the stream is not yet complete (still recording).
     *
     * On failure, throws an exception or returns YDataStream.DATA_INVALID.
     */
     ...
     * this method will always return YDataStream.DATA_INVALID.
     *
     * @return {Promise<number>} a floating-point number corresponding to the largest value,
     *         or YDataStream.DATA_INVALID if the stream is not yet complete (still recording).
     *
     * On failure, throws an exception or returns YDataStream.DATA_INVALID.
     */
     ...
     * if not yet done.
     *
     * @return {Promise<number[][]} a list containing as many elements as there are rows in the
     ...
     *
     * @return {Promise<number>} a floating-point number
     *
     * On failure, throws an exception or returns YDataStream.DATA_INVALID.
     */
...
class YDataSet
{
     ...
     *
     * @return {Promise<string>} a string that uniquely identifies the function (ex: THRMCPL1-123456.temperature1)
     *
     ...
     *
     * @return {Promise<string>} a string that identifies the function (ex: temperature1)
     */
     ...
     *
     * @return {Promise<string>} a string that represents a physical unit.
     *
     ...
     *
     * @return {Promise<number>} an integer in the range 0 to 100 (percentage of completion),
     ...
     *
     * @return {Promise<YMeasure>} an YMeasure object
     */
    async get_summary()
    {
     ...
     * been called for the first time.
     *
     * @return {Promise<YMeasure[]} a table of records, where each record depicts the
...
class YConsolidatedDataSet
{
     ...
     *
     * @param sensorNames {string[]} : array of logical names or hardware identifiers of the sensors
     ...
     * object.
     *
     * @param datarec {number[]} : array of floating point numbers, that will be filled by the
    ...
    async nextRecord(datarec)
    {
        ...
        if (globprogress > 0) {
            globprogress = parseInt(globprogress / this._nsensors, 10);
            if (globprogress > 99) {
                globprogress = 99;
            }
        }
...
YoctoLibExport('YConsolidatedDataSet', YConsolidatedDataSet);
...
class YDevice
{
    ...
    /** Return the root URL used to access a device (including the trailing slash but without user and pass)
     *
     * @returns {string}
     */
    imm_getSafeRootUrl()
    {
        ...
        let at = rootUrl.indexOf('@');
        let ds = rootUrl.indexOf('//');
        if (ds > 0 && ds < at) {
            rootUrl = this._rootUrl.substring(0, ds + 2) + this._rootUrl.substring(at + 1);
        }
        return rootUrl;
    }
    ...
    async requestAPI(int_msValidity)
    {
        ...
        try {
            ...
            if(Array.isArray(yreq.obj_result)) {
                if(precooked.module) {
                    yreq.obj_result = this.imm_jzon2json(yreq.obj_result, precooked);
                    if(yreq.obj_result.module &&
                        yreq.obj_result.module.serialNumber === precooked.module.serialNumber &&
                        yreq.obj_result.module.firmwareRelease === precooked.module.firmwareRelease) {
...
class YFirmwareUpdate
{
    ...
    static async GetBootLoaderInfoInContext(yctx, serial)
    {
        let hubs = yctx._hubs;

        for (let i = 0; i < hubs.length; i++) {
            let ldr = await hubs[i].getBootloaderInfo(serial);
            if (ldr['serial'] == serial){
                return ldr
            }
        }
        return {};
    ...
    static async GetBootLoaderInfo(serial)
    {
        return YFirmwareUpdate.GetBootLoaderInfoInContext(YAPI, serial);
    }
     ...
     *
     * @return {Promise<string[]} an array of strings containing the serial numbers of devices in
     ...
     * @param minrelease {number} : a positive integer
     *
     * @return {Promise<string>} : the path of the byn file to use, or an empty string if no byn files
     ...
     *
     * @return {Promise<string>} a string  with the latest progress message, or the error message.
     */
...
class YHub
{
    ...
    async _getStrAttr_internal(attrName)
    {
        ...
        switch (attrName) {
            ...
            case "connectionUrl":
                let url = hub.urlInfo.url;
                let at = url.indexOf('@');
                let ds = url.indexOf('//');
                if (ds > 0 && ds < at) {
                    url = url.substring(0, ds + 2) + url.substring(at + 1);
                }
                return url;
            case "serialNumber":
                return hub.imm_getHubSerial();
            case "errorMessage":
                return hub.imm_get_errorMessage()
            default:
                return "";
        }
    }
    ...
    async _getIntAttr_internal(attrName)
    {
        ...
        switch (attrName) {
            ...
            case "isReadOnly":
                return hub.imm_hasRwAccess() ? 0 : 1;
            case "networkTimeout":
                return hub.stalledTimeoutMs;
            case "errorType":
                return hub.imm_get_errorType();
            default:
                return -1;
        }
    }
    ...
    {
        await this._setIntAttr_internal(attrName,value);
    }
     ...
     *
     * @return {Promise<boolean>} true if the hub has not been unregistered.
     */
    async isInUse()
    {
     ...
     *
     * @return {Promise<boolean>} true if the hub is currently connected.
     */
    async isOnline()
    {
     ...
     *
     * @return {Promise<boolean>} true if it is not possible to change attributes on this hub.
     */
    async isReadOnly()
    {
     ...
     *
     * @param networkMsTimeout {number} : the network connection delay in milliseconds.
     * @noreturn
     */
     ...
     * afterward for each specific hub if necessary.
     *
     * @return {number} the network connection delay in milliseconds.
     */
     ...
     * exceptions disabled.
     *
     * @return {Promise<number>} a number corresponding to the code of the latest error that occurred while
     ...
     * exceptions disabled.
     *
     * @return {Promise<string>} a string corresponding to the latest error message that occured while
     *         using the hub object
     */
     ...
     *
     * @return {Promise<any>} the object stored previously by the caller.
     */
    async get_userData()
    {
     ...
     * disposal of the caller to store a context.
     *
     * @param data {any} : any kind of object to be stored
     * @noreturn
     */
     ...
     * next hubs.
     *
     * @return {YHub | null} a pointer to a YHub object, corresponding to
     ...
     * serial of the hub.
     *
     * @param url {string} : The url or serial of the hub.
     *
     * @return {Promise<YHub | null>} a pointer to a YHub object, corresponding to
     *         the first hub currently in use by the API, or a
     *         null pointer if none has been registered.
     */
...
class YFunction
{
     ...
     *
     * @return {Promise<string>} a string corresponding to the logical name of the function
     *
     ...
     *
     * @return {Promise<string>} a string corresponding to a short string representing the current state
     * of the function
     *
     ...
     *
     * @param func {string} : a string that uniquely characterizes the function, for instance
     *         MyDevice..
     *
     ...
     *
     * @param callback {YFunction.ValueCallback | null} : the callback function to call, or a null
     * pointer. The callback function should take two
     ...
     *
     * @return {Promise<string>} a string with the value of the the attribute
     *
     ...
     *
     * @return {Promise<boolean>} true if the function is write-protected or not online.
     */
    async isReadOnly()
    {
     ...
     *
     * @return {Promise<string>} a string corresponding to the serial number of the module, as set by the factory.
     *
    ...
    _is_valid_pass(passwd)
    {
        /** @type {string} **/
        let tmp;
        if ((passwd).length > this._yapi.HASH_BUF_SIZE) {
            tmp = 'Password too long (max '+String(Math.round(this._yapi.HASH_BUF_SIZE))+' chars) :'+passwd;
     ...
     * a debugger.
     *
     * @return {Promise<string>} a string that describes the function
     ...
     *
     * @return {Promise<string>} a string that uniquely identifies the function (ex: RELAYLO1-123456.relay1)
     *
     ...
     * relay1
     *
     * @return {Promise<string>} a string that identifies the function (ex: relay1)
     *
     ...
     *
     * @return {Promise<string>} a string that uniquely identifies the function using logical names
     ...
     *
     * @param bin_json {Uint8Array}
     * @param str_key {string}
     * @return {string}
     **/
    imm_json_get_key(bin_json, str_key)
    {
        var loadval = JSON.parse(this._yapi.imm_bin2str(bin_json));
    ...
    {
        var json = JSON.parse(this._yapi.imm_bin2str(bin_json));
     ...
     *
     * @return {Promise<boolean>} true if the function can be reached, and false otherwise
     */
    async isOnline()
    {
     ...
     * YModule is not shown as on-line.
     *
     * @return {Promise<YModule>} an instance of YModule
     */
    async get_module()
    {
     ...
     *
     * @return {Promise<string>} an identifier of type YFUN_DESCR.
     *
...
class YModule extends YFunction
{
     ...
     *
     * @return {Promise<number>} the number of functions on the module
     *
     * On failure, throws an exception or returns a negative error code.
     */
    async functionCount()
     ...
     *
     * @return {Promise<string>} a string corresponding to the unambiguous hardware identifier of the
     ...
     * starting at 0 for the first function.
     *
     * @return {Promise<string>} a string corresponding to the type of the function.
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     * starting at 0 for the first function.
     *
     * @return {Promise<string>} a string corresponding to the base type of the function
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     * starting at 0 for the first function.
     *
     * @return {Promise<string>} a string corresponding to the logical name of the requested module function
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     * starting at 0 for the first function.
     *
     * @return {Promise<string>} a short string (up to 6 characters) corresponding to the advertised value
     * of the requested module function
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     *
     * @return {Promise<string>} a string corresponding to the logical name of the module
     *
    ...
    async get_url_internal()
    {
        ...
        return lockdev.imm_getSafeRootUrl();
    }
     ...
     *
     * @return {Promise<string>} a string corresponding to the commercial name of the module, as set by the factory
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the USB device identifier of the module
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the release number of the module hardware,
     ...
     *
     * @return {Promise<string>} a string corresponding to the version of the firmware embedded in the module
     *
     ...
     *
     * @return {Promise<number>} a value among YModule.PERSISTENTSETTINGS_LOADED,
     * YModule.PERSISTENTSETTINGS_SAVED and YModule.PERSISTENTSETTINGS_MODIFIED corresponding to the
     * current state of persistent module settings
     *
     ...
     *
     * @return {Promise<number>} either YModule.BEACON_OFF or YModule.BEACON_ON, according to the state of
     * the localization beacon
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the number of milliseconds spent since the
     ...
     *
     * @return {Promise<number>} an integer corresponding to the current consumed by the module on the USB
     * bus, in milli-amps
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the remaining number of seconds before the
     * module restarts, or zero when no
     *         reboot has been scheduled
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the value previously stored in this attribute
     *
     ...
     *
     * @param yctx {YAPIContext} : a YAPI context
     * @param func {string} : a string that uniquely characterizes the module, for instance
     *         MyDevice.module.
     *
     * @return {YModule} a YModule object allowing you to drive the module.
     */
    ...
    async revertFromFlash()
    {
        ...
        res = await this.set_persistentSettings(YModule.PERSISTENTSETTINGS_LOADED);
        if (!(res==YAPI_SUCCESS)) {
     ...
     *
     * @param callback {YModule.LogCallback | null} : the callback function to call, or a null pointer.
     ...
     *
     * @param callback {YModule.ConfigChangeCallback | null} : a procedure taking a YModule parameter, or null
     ...
     *
     * @param callback {YModule.BeaconCallback | null} : The callback function to call, or null to unregister a
     ...
     *
     * @return {Promise<string>} the path of the byn file to use or a empty string if no byn files matches
     * the requirement
     *
     ...
     *
     * @return {Promise<YFirmwareUpdate>} a YFirmwareUpdate object or NULL on error.
     */
     ...
     *
     * @return {Promise<Uint8Array>} a binary buffer with all the settings.
     *
     ...
     *
     * @return {Promise<boolean>} true if the device has the function identifier
     */
     ...
     *
     * @return {Promise<string[]} an array of strings.
     */
            ...
            if (paramVer == 2) {
                ...
                if (paramVer == 1) {
                    ...
                    for (let ii_0 of words_str) {
                        words.push(this._yapi.imm_atoi(ii_0));
                    }
                    if (param == '' || (words[0] > 10)) {
                        paramScale = 0;
                    }
                    ...
                    if (paramVer == 0) {
                        ratio = this._yapi.imm_atof(param);
                        if (ratio > 0) {
                            calibData.push(0.0);
                            calibData.push(0.0);
     ...
     * HTTP callback mode.
     *
     * @param filename {string} : the name of the file to upload at the next HTTP callback
     *
     ...
     *
     * @return {Promise<string>} a string that uniquely identifies the module
     */
    async get_hardwareId()
    {
        /** @type {string} **/
     ...
     *
     * @return {Promise<Uint8Array>} a binary buffer with the file content
     *
     ...
     * exceeds 1536 bytes.
     *
     * @return {Promise<Uint8Array>} a binary buffer with module icon, in png format.
     *         On failure, throws an exception or returns an empty content.
     */
    async get_icon2d()
    {
     ...
     *
     * @return {Promise<string>} a string with last logs of the module.
     ...
     *
     * @return {Promise<string[]} an array of strings containing the sub modules.
     */
     ...
     *
     * @return {Promise<string>} a string with the serial number of the YoctoHub or an empty string
     */
    async get_parentHub()
    {
     ...
     *
     * @return {Promise<string>} a string with the URL of the module.
     */
    async get_url()
    {
     ...
     *
     * @return {YModule | null} a pointer to a YModule object, corresponding to
...
class YModuleProxy extends YFunctionProxy
{
    ...
    /**
     * Returns the commercial name of the device, with the hardware revision indicator
     * if relevant, as preset by the factory.
     */
    get_productNameAndRevision()
    {
        let prodname = this.liveFunc._productName;
        let prodrel = this.liveFunc._productRelease;
        if (prodrel > 1) {
            return prodname+' rev. '+String.fromCharCode(64+prodrel);
        }
        return prodname;
    }
    ...
    {
        this.liveFunc.addFileToHTTPCallback(filename);
        return YAPI_SUCCESS;
    }
...
class YSensor extends YFunction
{
     ...
     *
     * @return {Promise<string>} a string corresponding to the measuring unit for the measure
     *
     ...
     *
     * @return {Promise<number>} a floating point number corresponding to the current value of the
     * measure, in the specified unit, as a floating point number
     *
     ...
     *
     * @return {Promise<number>} a floating point number corresponding to the minimal value observed for
     * the measure since the device was started
     *
     ...
     *
     * @return {Promise<number>} a floating point number corresponding to the maximal value observed for
     * the measure since the device was started
     *
     ...
     *
     * @return {Promise<number>} a floating point number corresponding to the uncalibrated, unrounded raw
     ...
     *
     * @return {Promise<string>} a string corresponding to the datalogger recording frequency for this
     * function, or "OFF"
     *         when measures are not stored in the data logger flash memory
     *
     ...
     *
     * @return {Promise<string>} a string corresponding to the timed value notification frequency, or "OFF" if timed
     *         value notifications are disabled for this function
     *
     ...
     *
     * @return {Promise<number>} a value among YSensor.ADVMODE_IMMEDIATE, YSensor.ADVMODE_PERIOD_AVG,
     ...
     * Remember to call the saveToFlash() method of the module if the modification must be kept.
     *
     * @return {Promise<number>} a floating point number corresponding to the resolution of the measured values
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the sensor state code, which is zero when
     ...
     * call registerHub() at application initialization time.
     *
     * @param func {string} : a string that uniquely characterizes the sensor, for instance
     *         MyDevice..
     *
     ...
     *
     * @return {Promise<boolean>} true if the sensor can provide an up-to-date measure, and false otherwise
     */
    async isSensorReady()
    {
        try {
     ...
     * should not be freed.
     *
     * @return {Promise<YDataLogger | null>} an YDatalogger object, or null on error.
     */
     ...
     *         to include any measure, without ending limit.
     *
     * @return {Promise<YDataSet>} an instance of YDataSet, providing access to historical
     ...
     * one of these two functions periodically. To unregister a callback, pass a null pointer as argument.
     *
     * @param callback {YSensor.TimedReportCallback | null} : the callback function to call, or a null
     * pointer. The callback function should take two
     ...
     * and a hardwareID or a logical name.
     *
     * @return {YSensor | null} a pointer to a YSensor object, corresponding to
...
class YDataLogger extends YFunction
{
     ...
     *
     * @return {Promise<number>} an integer corresponding to the current run number, corresponding to the
     * number of times the module was
     *         powered on with the dataLogger enabled at some point
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the Unix timestamp for current UTC time, if known
     *
     ...
     *
     * @return {Promise<number>} a value among YDataLogger.RECORDING_OFF, YDataLogger.RECORDING_ON and
     ...
     *
     * @return {Promise<number>} either YDataLogger.AUTOSTART_OFF or YDataLogger.AUTOSTART_ON, according
     * to the default activation state of the data logger on power up
     *
     ...
     *
     * @return {Promise<number>} either YDataLogger.BEACONDRIVEN_OFF or YDataLogger.BEACONDRIVEN_ON,
     * according to true if the data logger is synchronised with the localization beacon
     *
     ...
     *
     * @return {Promise<number>} an integer corresponding to the percentage of datalogger memory in use
     *
     ...
     * call registerHub() at application initialization time.
     *
     * @param func {string} : a string that uniquely characterizes the data logger, for instance
     *         LIGHTMK4.dataLogger.
     *
     ...
     * version 13000.
     *
     * @return {Promise<YDataSet[]} a list of YDataSet object.
     *
     ...
     * and a hardwareID or a logical name.
     *
     * @return {YDataLogger | null} a pointer to a YDataLogger object, corresponding to
...
class YGenericHub
{
    constructor(obj_yapi, var_urlInfo)
    {
        ...
        this._knownUrls = [];
        /** @member {number} **/
        this.stalledTimeoutMs = obj_yapi._networkTimeoutMs;               // hub-specific timeout for detecting stalled connections
    }

    imm_getHubSerial()
    {
        return this.serialByYdx[0];
    }

    imm_get_errorMessage()
    {
        // fixme port TypeScript implementation
        return "";
    }
    ...
    get_knownUrls()
    {
        let res = this._knownUrls;
        res.push(this.urlInfo.orgUrl);
        return res;
    }
    ...
    imm_testHubAgainLater()
    {
        ...
        if (this._connectionType == this._HUB_PREREGISTRED && this._hubAdded) {
    ...
    async getBootloaderInfo(serial)
    {
        let res = {
            bld_version: 0,
            yfsSignature:0,
        ...
        let yreq = await this.request('GET', '/flash.json?a=detail&s=' + serial, null, 1);
        if (yreq.errorType != YAPI_SUCCESS) {
            return this._throw(yreq.errorType, yreq.errorMsg, res);
        }
        let info = null;
        try {
            info = JSON.parse(YAPI.imm_bin2str(yreq.bin_result));
        } catch (err) {
            return this._throw(YAPI_IO_ERROR, "Invalid response", res);
        }
        res['serial'] = serial;
        if (info['bld']) {
            res.bld_version = info['bld'];
        }
        if (info['fw']) {
            res.fw_rev = info['fw'];
        }
        if (info['yfs']) {
            res.yfsSignature = info['yfs'];
        }
        if (info['st']) {
            res.blr_start = info['st'];
        }
        if (info['res']) {
            res.blr_result = info['res'];
        }
        if (info['err']) {
            res.panic_origin = info['err'];
        }
        if (info['irr']) {
            res.panic_irritant = info['irr'];
        }
        return res;
    }
...
class YHttpHub extends YGenericHub
{
    constructor(obj_yapi, var_urlInfo)
    {
        ...
        /** @member {number} **/
        this.currPos           = 0;            // current position in XMLHTTPRequest stream, when reply comes in parts
    ...
    //
    imm_sendXHR(xmlHttpRequest, method, uri, obj_body,
    ...
    async testHub(mstimeout, errmsg)
    {
        ...
        if(!this.infoJson) {
            if(!await new Promise((resolve, reject) => {
                ...
                let safe_url = this.urlInfo.url.replace('#', '%23');
                this.imm_sendXHR(xhr, 'GET', safe_url + 'info.json', null,
                    () => {
                        if (xhr.readyState === 4) {
                            if(xhr.status === 200) {
            ...
            this.notbynOpenPromise = new Promise((resolve, reject) => {
                if (mstimeout) {
                    this.notbynOpenTimeoutObj = setTimeout(() => {
                        resolve({ errorType: YAPI_TIMEOUT, errorMsg: "Timeout on HTTP connection" });
                        this.disconnect();
                    }, mstimeout);
                }
                this.notbynTryOpen = () => {
    ...
    imm_sendPendingRequest(tcpchan)
    {
        ...
        while(yreq) {
            ...
            if (yreq.devUrl.indexOf('/testcb.txt') >= 0) {
                ...
                mstimeout = this._YIO_10_MINUTES_TCP_TIMEOUT;
            } else if (yreq.devUrl.indexOf('/Yv4wI.js') >= 0) {
                mstimeout = this._YIO_10_MINUTES_TCP_TIMEOUT;
            }
...
//--- (generated code: YFunction return codes)
        ...
        this.SSL_ERROR             = -15;     // Error reported by mbedSSL
        this.RFID_SOFT_ERROR       = -16;     // Recoverable error with RFID tag (eg. tag out of reach), check YRfidStatus for details
        this.RFID_HARD_ERROR       = -17;     // Serious RFID error (eg. write-protected, out-of-boundary), check YRfidStatus for details
        this.BUFFER_TOO_SMALL      = -18;     // The buffer provided is too small
        this.DNS_ERROR             = -19;     // Error during name resolutions (invalid hostname or dns communication error)
        this.SSL_UNK_CERT          = -20;     // The certificate is not correctly signed by the trusted CA
        this.UNCONFIGURED          = -21;     // Remote hub is not yet configured
        // TLS/SSL definitions
        this.NO_TRUSTED_CA_CHECK   = YAPI_NO_TRUSTED_CA_CHECK; // Disables certificate checking
        this.NO_EXPIRATION_CHECK   = YAPI_NO_EXPIRATION_CHECK; // Disables certificate expiration date checking
        this.NO_HOSTNAME_CHECK     = YAPI_NO_HOSTNAME_CHECK;   // Disable hostname checking
        this.LEGACY                = YAPI_LEGACY;              // Allow non-secure connection (similar to v1.10)
//--- (end of generated code: YFunction return codes)
        this._deviceListValidityMs = 10000;
        this._networkTimeoutMs     = 20000;
//--- (generated code: YAPIContext constructor)
     ...
     *
     * @param logfun {YLogCallback} : a procedure taking a string parameter, or null
     *         to unregister a previously registered  callback.
     */
    ...
    async updateDeviceList_process(hub, hubDev, whitePages, yellowPages)
    {
        ...
        if (this._arrivalCallback != undefined && hub._firstArrivalCallback) {
        ...
        for (var idx = 0; idx < nrows; idx++) {
            ...
            if (ev.length >= 3 && firstCode >= NOTIFY_NETPKT_CONFCHGYDX && firstCode <= NOTIFY_NETPKT_TIMEAVGYDX) {
                ...
                if (serial && this._devs[serial]) {
                    var funcid = (funydx == 0xf ? 'time' : this._devs[serial].imm_functionIdByFunYdx(funydx));
                    if (funcid != '') {
                        let dev;
                        value = ev.slice(3);
                        switch (firstCode) {
    ...
    {
        return "error: Not supported in EcmaScript";
    }
     ...
     *
     * @param deviceListValidity {number} : nubmer of seconds between each enumeration.
     * @noreturn
     */
     ...
     * Note: you must call this function after yInitAPI.
     *
     * @return {Promise<number>} the number of seconds between each enumeration.
     */
     ...
     * empty string.
     *
     * @return {Promise<string>} a string containing the path of the YAPI dynamic library.
     */
     ...
     *
     * @param force {boolean} : if true, overwrites any existing rule.
     *
     * @return {Promise<string>} an empty string if the rule has been added.
     *
     * On failure, returns a string that starts with "error:".
     */
     ...
     *
     * @param url {string} : the root URL of the VirtualHub V2 or HTTP server.
     * @param mstimeout {number} : the number of milliseconds available to download the certificate.
     *
     * @return {Promise<string>} a string containing the certificate. In case of error, returns a string
     * starting with "error:".
     */
     ...
     *
     * @param certificate {string} : a string containing one or more certificates.
     *
     * @return {Promise<string>} an empty string if the certificate has been added correctly.
     ...
     *
     * @param certificatePath {string} : the path of the file containing all certificates in PEM format.
     *
     * @return {Promise<string>} an empty string if the certificate has been added correctly.
     *         In case of error, returns a string starting with "error:".
     */
     ...
     *
     * @param opts {number} : The options are YAPI.NO_TRUSTED_CA_CHECK,
     *         YAPI.NO_EXPIRATION_CHECK, YAPI.NO_HOSTNAME_CHECK.
     *
     * @return {Promise<string>} an empty string if the options are taken into account.
    ...
    {
        await this.SetNetworkTimeout_internal(networkMsTimeout);
    }
     ...
     *
     * @return {Promise<number>} the network connection delay in milliseconds.
     */
     ...
     * Note: This function must be called after yInitAPI .
     *
     * @return {Promise<number>} an integer corresponding to the validity attributed to the
     *         loaded function parameters, in milliseconds
     */
    ...
    nextHubInUseInternal_internal(int_hubref)
    {
        let nextref = int_hubref < 0 ? 0 : int_hubref + 1;
        while (nextref < this._hubs.length && !this._hubs[nextref].isEnabled()) {
            nextref++;
        }
        if (nextref >= this._hubs.length) {
            return null;
        }
    ...
    getGenHub(hubref)
    {
        if (hubref < 0 || hubref >= this._hubs.length) {
            return null;
        }
        return this._hubs[hubref];
    }

    _findYHubFromCache(int_hubref)
    {
        return this._yhub_cache[int_hubref.toString()];
    }

    _addYHubToCache(int_hubref, yhub_obj)
    {
        this._yhub_cache[int_hubref.toString()] = yhub_obj;
    }
     ...
     *
     * @return {Promise<string>} a character string describing the library version.
     */
    async GetAPIVersion()
    {
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'2.1.15726';
    }
    ...
    async RegisterHub(url, errmsg)
    {
        ...
        if(url === "usb") {
            return this._yapi._throw(YAPI_NOT_SUPPORTED, 'Use the VirtualHub on 127.0.0.1 to access local USB devices', YAPI_NOT_SUPPORTED);
        }
     ...
     *
     * @param errmsg {YErrorMsg | null} : a string passed by reference to receive any error message.
     *
     * @return {Promise<number>} YAPI.SUCCESS when the call succeeds.
     *
     * On failure returns a negative error code.
     ...
     *
     * @param callback {Function} : the function to call after the timeout occurs.
     *         On Microsoft Internet Explorer, the callback must
     *         be provided as a string to be evaluated.
     * @param ms_timeout {number} : an integer corresponding to the duration of the
     ...
     *
     * @return {Promise<boolean>} true if the name is valid, false otherwise.
     */
     ...
     *
     * @param arrivalCallback {YDeviceUpdateCallback | null} : a procedure taking a YModule parameter, or null
     *         to unregister a previously registered  callback.
     */
     ...
     * is running. You will have to call this function on a regular basis.
     *
     * @param removalCallback {YDeviceUpdateCallback | null} : a procedure taking a YModule parameter, or null
     *         to unregister a previously registered  callback.
     */
     ...
     *
     * @param hubDiscoveryCallback {YHubDiscoveryCallback} : a procedure taking two string parameter, the serial
