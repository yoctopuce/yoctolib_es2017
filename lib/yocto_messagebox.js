/*********************************************************************
 *
 * $Id: yocto_messagebox.js 27510 2017-05-17 13:55:12Z mvuilleu $
 *
        ...
        while (i+1 < udhlen) {
            ...
            if (i + ielen <= udhlen) {
                if ((iei == 0) && (ielen == 3)) {
                    ...
                    sig = this._orig+'-'+this._dest+'-'+('00'+(this._mref).toString(16)).slice(-2)+'-'+('00'+(this._udh[i]).toString(16)).slice(-2);
                    this._aggSig = sig;
                ...
                if ((iei == 8) && (ielen == 4)) {
                    ...
                    sig = this._orig+'-'+this._dest+'-'+('00'+(this._mref).toString(16)).slice(-2)+'-'+('00'+(this._udh[i]).toString(16)).slice(-2)+''+('00'+(this._udh[i+1]).toString(16)).slice(-2);
                    this._aggSig = sig;
        ...
        if (this._alphab == 0) {
            ...
            while (i < udlen) {
                if (nbits == 7) {
                    ...
                    thisb = pdu[rpos];
                    rpos = rpos + 1;
                    this._udata.set([((carry) | ((((((thisb) << (nbits)))) & (127))))], i);
                    carry = ((thisb) >> ((7 - nbits)));
                    nbits = nbits + 1;
                }
                i = i + 1;
...
class YMessageBox extends YFunction
{
    ...
    async get_syncProxy()
    {
        ...
        let res = new YMessageBoxProxy(this);
