...
class YSdi12SensorInfo
{
     ...
     *
     * @return {Promise<boolean>} the sensor state.
     */
    async isValid()
    {
...
class YSdi12Port extends YFunction
{
     ...
     *
     * @return {Promise<YSdi12SensorInfo>} the reply returned by the sensor, as a YSdi12SensorInfo object.
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     * This function is intended to be used when the serial port is configured for 'SDI-12' protocol.
     *
     * @return {Promise<YSdi12SensorInfo[]} all the information from every connected sensor, as an array
     * of YSdi12SensorInfo object.
     *
     * On failure, throws an exception or returns an empty string.
     */
    async discoverAllSensors()
    {
        /** @type {YSdi12SensorInfo[]} **/
        let sensors = [];
        /** @type {string[]} **/
        let idSens = [];
        /** @type {string} **/
    ...
    async readSensor(sensorAddr,measCmd,maxWait)
    {
        ...
        valdouble = this._yapi.imm_atof(split[1]);
        res.push(valdouble);
        i = 1;
        while (i < tab.length) {
            valdouble = this._yapi.imm_atof(tab[i]);
            res.push(valdouble);
            i = i + 1;
        }
        return res;
     ...
     *
     * @return {Promise<YSdi12SensorInfo>} the sensor address and information , as a YSdi12SensorInfo object.
     *
     * On failure, throws an exception or returns an empty string.
     */
     ...
     *
     * @return {Promise<YSdi12SensorInfo>} the reply returned by the sensor, as a YSdi12Port object.
     *
     * On failure, throws an exception or returns an empty string.
     */
