...
class YCounter extends YSensor
{
     ...
     *
     * @return {Promise<number>} either YCounter.DECIMALMODE_FALSE or YCounter.DECIMALMODE_TRUE, according
     * to a value indicating if the senseur compute whole or fractional values
     *
     ...
     *
     * @param newval {number} : either YCounter.DECIMALMODE_FALSE or YCounter.DECIMALMODE_TRUE, according
     * to the sensor's operating mode so that it computes integer or decimal values
     *
     ...
     *
     * @return {Promise<number>} YAPI.SUCCESS if the call succeeds. Please note that this function only resets
...
class YCounterProxy extends YSensorProxy
{
    ...
    get_decimalMode()
    {
        return this.liveFunc._decimalMode;
    }
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    set_decimalMode(newval)
    {
        this.liveFunc.set_decimalMode(newval);
