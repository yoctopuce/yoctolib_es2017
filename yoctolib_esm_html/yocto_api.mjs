/*********************************************************************
 *
 * $Id: yocto_api.js 41769 2020-09-03 17:34:23Z mvuilleu $
 *
...
// Uncomment the line below if your environment requires specific modules to provide system-access function
// (as in yoctolib_esm_nodejs for instance)
//
// import * as system_env from './system_env.mjs';

/**
 * Default system_env support class, for use in a browser
 *
 * This code is intentionally built into the library to keep yocto_api usable as a monolithic file
 * It can be overriden by an import of system_env at top of this file
 */
if(typeof system_env === "undefined") {
    if (typeof window === "undefined") {
        throw new Error("no system_env package imported and 'window' is not defined");
    }
    if(typeof window.XMLHttpRequest === "undefined") {
        throw new Error("'XMLHttpRequest' is expected to be defined globally in the browser");
    }
    if(typeof window.WebSocket === "undefined") {
        throw new Error("'WebSocket' is expected to be defined globally in the browser");
    }
    // We are in a browser, define defaults value
    // We are in a browser, define default
    window.system_env = {
        isNodeJS: false,
        require: async (pkgname) => { throw new Error("Unexpected use of 'require' in browser environment"); },
...
export class YDataSet
{
    ...
    async loadMore()
    {
        ...
        try {
            return this.processMore(this._progress, await this._parent._download(url));
        } catch (e) {
            return this.processMore(this._progress, await this._parent._download(url));
        }
    }
...
export class YFirmwareUpdate
{
    ...
    async _processMore_internal(newupdate)
    {
        ...
        if (typeof this._firmwarepath == 'string' && this._firmwarepath.indexOf('yoctopuce.com') >= 0) {
            ...
            bytes = await system_env.downloadfile(this._firmwarepath);
        } else {
            this.imm_progress(1, 'Loading firmware');
            bytes = await system_env.loadfile(this._firmwarepath);
        }
        /** @type {YFirmwareFile} **/
    ...
    static async checkFirmware_r(file, serial_base, force)
    {
        ...
        let bynfile = await system_env.loadfile(file);
    ...
    static async CheckFirmwareEx(serial,path,minrelease,force)
    {
        ...
        if (typeof path == 'string' && path.indexOf('yoctopuce.com') >= 0) {
            try {
                let data = await system_env.downloadfile('http://www.yoctopuce.com/FR/common/getLastFirmwareLink.php?serial=' + serial);
    ...
    {
        return this.GetAllBootLoaders_internal();
    }
    ...
    {
        return this.GetAllBootLoadersInContext_internal(yctx);
    }
    ...
    {
        return this.CheckFirmware_internal(serial,path,minrelease);
    }
...
export class YFunction
{
    ...
    static _E(arr_attrlist)
    {
        ...
        for(let i = 0; i < arr_attrlist.length; i++) {
            ...
            this.prototype[getMethod] = async function() { return this._g(attrname); };
        }
...
export class YModule extends YFunction
{
     ...
     *
     * @return {number} an integer corresponding to the release number of the module hardware,
    ...
    async saveToFlash()
    {
        return this.set_persistentSettings(YModule.PERSISTENTSETTINGS_SAVED);
    }
    ...
    {
        return this.set_persistentSettings(YModule.PERSISTENTSETTINGS_LOADED);
    }
    ...
    async log(text)
    {
        return this._upload('logs.txt', this._yapi.imm_str2bin(text));
    }
...
export class YModuleProxy extends YFunctionProxy
{
    constructor(obj_func)
    {
        super(obj_func);
...
export class YSensor extends YFunction
{
     ...
     *
     * @return {number} a floating point number corresponding to the uncalibrated, unrounded raw value returned by the
     *         sensor, in the specified unit, as a floating point number
     *
...
//
/** @extends {YFunctionProxy} **/
export class YSensorProxy extends YFunctionProxy
{
    constructor(obj_func)
    {
        super(obj_func);
...
export class YDataLogger extends YFunction
{
    ...
    {
        return this.set_clearHistory(YDataLogger.CLEARHISTORY_TRUE);
    }
    ...
    async get_dataSets()
    {
        return this.parse_dataSets(await this._download('logger.json'));
    }
...
//
/** @extends {YFunctionProxy} **/
export class YDataLoggerProxy extends YFunctionProxy
{
    constructor(obj_func)
    {
        super(obj_func);
...
class YWebSocketHub extends YGenericHub
{
    ...
    {
        this.websocket = new system_env.WebSocket(str_url);
            ...
            if(ystream == this._YSTREAM_META) {
                ...
                switch (metatype) {
                    case this._USB_META_WS_ANNOUNCE:
                        ...
                        system_env.getRandomValues(nonce);
...
//--- (generated code: YAPIContext constructor)
    ...
    imm_init()
    {
        ...
        this._beacons = {};
        /** @member {boolean} **/
        this._isNodeJS                  = system_env.isNodeJS;
        /** @member {function} **/
        this._nodeRequire               = system_env.require;
    }
    ...
    imm_GetAPIVersion()
    {
        return /* version number patched automatically */'1.10.42639';
    }
    ...
    {
        system_env.hookUnhandledRejection((reason, promise) => {
    ...
    async WebSocketJoin(ws, auth, closeCallback)
    {
        ...
        return this._hubs[0].websocketJoin(ws, auth, closeCallback);
    }
...
export let YAPI = new YAPIContext();
...
export async function yGetAPIVersion()
{ return YAPI.GetAPIVersion(); }

export async function yInitAPI(mode,errmsg)
{ return YAPI.InitAPI(mode,errmsg); }

export async function yFreeAPI()
{ return YAPI.FreeAPI(); }

export async function yDisableExceptions()
{ return YAPI.DisableExceptions(); }

export async function yEnableExceptions()
{ return YAPI.EnableExceptions(); }

export async function yRegisterHub(url,errmsg)
{ return YAPI.RegisterHub(url,errmsg); }

export async function yPreregisterHub(url,errmsg)
{ return YAPI.PreregisterHub(url,errmsg); }

export async function yUnregisterHub(url)
{ return YAPI.UnregisterHub(url); }

export async function yUpdateDeviceList(errmsg)
{ return YAPI.UpdateDeviceList(errmsg); }

export async function yHandleEvents(errmsg)
{ return YAPI.HandleEvents(errmsg); }

export async function ySleep(ms_duration, errmsg)
{ return YAPI.Sleep(ms_duration, errmsg); }

export function ySetTimeout(callback, ms_timeout, args)
{
...
export async function yCheckLogicalName(name)
{ return YAPI.CheckLogicalName(name); }

export async function yRegisterDeviceArrivalCallback(arrivalCallback)
{ return YAPI.RegisterDeviceArrivalCallback(arrivalCallback); }

export async function yRegisterDeviceChangeCallback(changeCallback)
{ return YAPI.RegisterDeviceChangeCallback(changeCallback); }

export async function yRegisterDeviceRemovalCallback(removalCallback)
{ return YAPI.RegisterDeviceRemovalCallback(removalCallback); }

export async function yRegisterCalibrationHandler(calibrationType, calibrationHandler)
